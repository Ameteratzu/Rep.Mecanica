...
@proveedor_bp.route('/buscar_proveedores')
def buscar_proveedores():
    term = request.args.get('term', '', type=str).strip()
    print(f"Término de búsqueda recibido: {term}")  # Para depurar
    ...
    if term == '':
        ...
        proveedores = Proveedor.query.filter(
            (Proveedor.ruc.ilike(term_like)) | 
            (Proveedor.razon_social.ilike(term_like)) |
            (Proveedor.telefono.ilike(term_like)) |
            (Proveedor.direccion.ilike(term_like))
        ).limit(20).all()
